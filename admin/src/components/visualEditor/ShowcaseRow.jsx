...
function ShowcaseRow({ items, index, reverse, onImageChange }) {
          ...
          <AnimatePresence mode="wait">
            ...
            >
              <EditableImage
                ...
                onChange={(url) => onImageChange(active, url)}
              />
            </motion.div>
          </AnimatePresence>
        </div>
          ...
          <div className="showcase-dots">
            {items.map((subItem, i) => (
              <button
                type="button"
                key={subItem.title || i}
                className={`showcase-dot ve-showcase-dot ${i === active ? 'is-active' : ''}`}
                aria-label={`Show story ${i + 1}: ${subItem.title}`}
                onClick={() => setActive(i)}
              />
            ))}
          </div>
